...
**Unified Object Inspector**
    ...
    when maximum canvas space is needed. Its top card follows the active tool
    or selected object, keeping image edit commands, selected-track
    propagation, and pending-review decisions in context.
...
**Faster Class Confirmation**
    ...
    labels and established single-class sessions bypass the picker. Box,
    Polygon, and Smart Select stay active after confirmation so repeated
    annotation does not require re-arming the tool; choose Select or press
    **Escape** while idle to end the session. The canvas chrome and status strip
    always state what the next gesture will do.

**One Completion Action**
    The command bar derives one primary action from the current document:
    **Done & Next** saves, marks an image verified, and only then advances;
    **Browse video** opens the track/frame overview; and **Review queue** opens
    the next pending suggestion directly on the canvas. Press **E** to complete
    an image. A failed or superseded save never navigates away, and explicit
    Save and Verify
    commands remain available from the application menu.
...
**Portable Whole-Video Propagation**
    Select a manual anchor to propagate that object from its contextual card;
    the card's **More** menu also exposes whole-frame scope. The integrated
    timeline shows elapsed time, the Anchor → Propagate → Review → Export
    stage, and cancellable progress. The portable OpenCV backend
    processes rectangles, polygons, and associated keypoints together, shows
    preview-only progress, protects later manual anchors, and commits generated
    observations as pending tracker suggestions with explicit gap records in
    one atomic undo step. Suggestions must be accepted before export.
    Editing generated geometry creates a manual correction first, then
...
**Continuous Video Review**
    **Review queue** keeps the affected frame and track selected. **Accept &
    Next** and **Reject** update one observation in one undo step and advance
    to the next live pending issue; **Previous** wraps through the same queue.
    ``Shift+Enter`` accepts, ``Backspace`` rejects, and the canvas keeps focus.
    Full-run review remains available from **More**. Pending and rejected
    suggestions are never included in accepted-frame export.
...
labelImg++ 4 requires Python 3.10 or newer and PyQt6 6.11.
...
2. **Create annotations**: Press **W** or click **Create RectBox**, then draw a box
3. **Label objects**: Confirm a class beside the box, then continue drawing; Box stays active
4. **Complete**: Press **E** for **Done & Next** to save, verify, and advance
5. **Navigate manually**: Use **D** (next) and **A** (previous) when needed
